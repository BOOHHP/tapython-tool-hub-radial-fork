...
| `ADMIN_USERNAME` | 未配置 | 单管理员登录用户名；未配置时 `/api/admin/*` 返回 `503`。 |
| `ADMIN_PASSWORD_HASH` | 未配置 | PBKDF2 密码哈希，不保存明文密码。 |
| `AUTH_SESSION_SECRET` | 未配置 | 管理员 Cookie Session 签名密钥。 |
| `ADMIN_SESSION_TTL_HOURS` | `12` | 管理员登录有效期，单位小时。 |
...
投稿路由面向投稿用户展示状态；审核与发布能力已移动到 `/api/admin/*`，并要求已配置的单管理员 Cookie Session。
...
本地开发和验证继续使用 `start-production.bat`。服务端更新部署建议使用 `scripts\deploy-server.bat`，它会先把服务端工作区强制对齐到远端分支，再调用 `start-production.bat` 启动，避免服务端 Codex 或手工操作留下的文件改动污染部署目录。

```bat
REM 默认：radial-fork master 10.2.13.8 5174 8787 dual
scripts\deploy-server.bat

REM 完整参数：[REMOTE] [BRANCH] [HOST] [WEB_PORT] [API_PORT] [MODE]
scripts\deploy-server.bat radial-fork master 10.2.13.8 5174 8787 dual
```

`deploy-server.bat` 只建议在 10.2.13.8 这类部署服务器使用，不建议在本地开发仓库运行；它会执行 `git reset --hard` 和 `git clean -fd`，但会保留 `.env`、`logs/` 和 `.tapython-tool-hub/`。
...
| `VITE_API_BASE_URL` | `auto` | 前端 API 地址。`auto` 会按浏览器主机名判断：本机访问走 `127.0.0.1:8787`，服务器 IP 访问走同主机 `:8787`。 |
| `ADMIN_USERNAME` | `admin` | 单管理员登录用户名。 |
| `ADMIN_PASSWORD_HASH` | *(空)* | PBKDF2 密码哈希；为空时后台管理不可用。 |
| `AUTH_SESSION_SECRET` | *(空)* | Cookie Session 签名密钥；为空时后台管理不可用。 |
| `ADMIN_SESSION_TTL_HOURS` | `12` | 管理员登录有效期，单位小时。 |

生成管理员密码哈希和 Session 密钥：
...
`start-production.bat` 启动的公盘/局域网部署会由 API 在运行时读取仓库根目录 `.env`；不要把 `.env` 提交到仓库。

前端默认不把某个 IP 写死进构建产物。通过 `localhost` 或 `127.*` 打开的页面会请求本机 API；通过 `10.2.13.8` 这类服务器 IP 打开的页面会请求同一服务器的 API。只有在 API 与页面不在同一主机时，才需要显式设置 `VITE_API_BASE_URL=http://api-host:8787`。
...
默认目标为 `radial-fork/master`，默认主机为 `10.2.13.8`。该脚本会检查 `.env` 管理员配置、拉取远端、重置 tracked 文件、清理未跟踪文件并保留运行数据，然后调用 `start-production.bat`。
...
下一阶段建议执行 execution roadmap 中的 Phase H：补数据库迁移脚本、本地 PostgreSQL 启动说明、route tests、PostgreSQL-first 工具读取，以及项目从单人运维扩展后的多管理员角色管理。
...
- 后台路由已具备单管理员 Cookie Session 保护；尚未实现多管理员用户和角色权限。
