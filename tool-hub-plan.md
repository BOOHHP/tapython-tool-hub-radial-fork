...
当前实现已经从“静态维护、基础可用”阶段演进到“API-backed hybrid”阶段：通过仓库中的 Markdown 工具文档维护工具数据，`packages/tooling` 生成兼容 API、manifest、Markdown 下载和引用资源；`apps/api` 承载 `/api/tools/*`、`/downloads/*`、投稿、审核和发布导出；`apps/web` 运行时通过后端 API 读取工具库。
...
5. 将 web build 与 API 服务发布到公司局域网环境，或按兼容产物进行静态部署。

网页投稿流程：

1. 工具作者在前端投稿页提交 Markdown 和引用文本资源。
2. 后端调用 `packages/tooling` 生成校验报告。
3. 审核者在投稿工作台通过或拒绝。
4. 审核通过后，后端导出 `data/tool-docs/<slug>/`、兼容 API、manifest、README、tool.md 和 downloads 产物。
5. 已发布版本保持不可变，同 slug 同版本会被校验阻止。
...
- 当前实现支持提交 Markdown 和 `@file:` 引用的文本资源，并在后端生成校验报告。
- 当前实现支持审核队列、通过/拒绝和审核通过后的兼容产物导出。
- 下一步补登录/权限、截图/二进制制品、对象存储和更细的后台管理视图。

## 搜索方案

当前实现使用前端本地筛选，但数据源来自后端兼容 API。短期继续保持前端内存筛选；工具数量上千或需要跨字段排序/权限过滤时，再切换到数据库全文搜索或专用搜索服务。

可选增强技术：`MiniSearch` 或 `Fuse.js`。
...
工具数量上千、需要多租户权限过滤或需要下载统计参与排序后，再切换到 `Meilisearch`、`Typesense` 或 PostgreSQL 全文搜索。
...
1. 作者在投稿页提交 Markdown 和 `@file:` 文本资源。
2. 后端调用 `packages/tooling` 解析 Markdown、生成 manifest 预览和校验报告。
3. 审核者查看校验结果并通过或拒绝。
4. 审核通过后导出 `data/tool-docs/<slug>/`、兼容 API 和 downloads。
5. 后续补登录、权限、截图/二进制制品上传和更完整的审核预览。
...
CLI、AI 助手提示词和 ZIP 安装的详细优化计划见 [docs/cli-installation-plan.md](docs/cli-installation-plan.md)。该计划参考 SkillHub `summarize` 的安装体验，但 Tool Hub 仍以 TAPython/Chameleon 编辑器工具包为分发对象。
...
- 数据库：`PostgreSQL`，本地无数据库时保留文件系统 fallback 便于试用。
- 共享契约：`packages/shared` 中的 TypeScript DTO 与 Zod schema。
- 内容处理：`packages/tooling`。
- 构建：`Node.js` 脚本与后端发布任务复用同一套 tooling。
- Markdown：`gray-matter`、`remark/rehype`。
- 代码高亮：`shiki` 或 `highlight.js`。
- 搜索：短期前端内存筛选，后续按规模切换数据库全文搜索或专用搜索服务。
- 制品：manifest + README + tool.md + 引用资源 + JSON 索引；ZIP 作为后续增强。
- 部署：`nginx` 静态 web + Fastify API，或通过 `VITE_API_BASE_URL` 指向独立 API 主机。

选择 `Ant Design` 的原因：Tool Hub 更像公司内部资产管理/工具管理平台，需要表格、筛选、上传、状态、审核和管理视图，Ant Design 成本最低。

### 后续运行化栈

当前后端已经引入，后续重点不是换栈，而是运行化：

- 迁移：`scripts/migration` 执行 PostgreSQL SQL 文件。
- 文件存储：短期本地磁盘/内网共享目录；中期 MinIO；云端再考虑 OSS/COS/S3/R2。
- 认证：短期内网管理 token；中期公司 SSO、LDAP、OAuth 或内网账号。
- 测试：Fastify route tests 覆盖 tools、downloads、submissions 和 health。
...
- `src/`：前端应用代码。
- `scripts/`：工具数据扫描、manifest 生成、下载制品生成、版本快照和 diff 脚本。
- `data/tool-docs/`：当前静态维护阶段的 Markdown-first 工具文档和引用资源。
- `data/tools/`：兼容旧 JSON 数据源。
- `public/`：静态资源。
- `docs/`：实施过程中的技术文档和接口说明。

## 执行版维护

当前文档仍然负责产品边界、阶段目标和总体架构原则。

面向工程落地的“目标目录树 + 包职责 + 迁移顺序”执行版，单独维护在 [docs/execution-roadmap.md](docs/execution-roadmap.md)。

这样拆分的原因是：

1. 本文档更适合描述产品级方案和演进方向。
2. 执行版文档更适合随着目录和包职责变化做高频更新。
3. 后续前后端重构、workspace 化和发布链路改造，不会把产品计划污染成纯工程笔记。
...
- MVP 优先支持局域网静态部署。
- 下一阶段采用单仓前后端共存结构，而不是前后端分仓。
- 后端演进优先采用 Node/TypeScript，并与前端共享 schema。
- 数据库将进入投稿、审核和发布主流程，但 Markdown/manifest 资产继续保留用于导入导出与审计。
